...
#Reading data
exp = read.csv('data_filtered.csv',header = T) 
...
##We choose the power 18, which is the lowest power for which the scale-free topology 
#fit index curve flattens out upon reaching a high value (in this case, roughly 0.90)

## One-step network construction and module detection
net = blockwiseModules(data, power = 18, corType = "pearson", networkType = "signed",
...
write.table(sum,'Gene count.txt',quote = F,row.names = F)
...
zygote = colMeans(MEs[1:2,])
octant = colMeans(MEs[3:4,])
globular = colMeans(MEs[5:6,])
heart = colMeans(MEs[7:8,])
torpedo = colMeans(MEs[9:10,])
bent  = colMeans(MEs[11:12,])
mature  = colMeans(MEs[13:14,])

MEs1 = data.frame(rbind(zygote,octant,globular,heart,torpedo,bent,mature))
colnames(MEs1) = ME_names
rm(zygote,octant,globular,heart,torpedo,bent,mature)

#trait data
trait = read.delim('trait.txt',header = F)
colnames(trait) = row.names(MEs1)
rownames(trait) = row.names(MEs1)
trait = as.data.frame(trait)
trait

moduleTraitCor = cor(MEs1,trait, use = "complete.obs")
...
labeledHeatmap(Matrix = moduleTraitCor,
               xLabels = c("Zygote","Octant","Globular","Heart",
                           "Torpedo","Bent","Mature"),
               yLabels = names(MEs1),
               ySymbols = names(MEs1),
               colorLabels = FALSE,
...
weight = as.data.frame(trait$zygote)
names(weight) = "weight"
...
geneTraitSignificance = as.data.frame(cor(data, weight, use = "p"));
...
par(mar=c(7,4,4,2))
heatmap.2(dt, col = cm.colors(256),  trace = "none", Colv = F, colsep=0:ncol(dt),
          rowsep=0:nrow(dt), sepcolor = c("lightgrey"),margins=c(8,8),
          dendrogram = "none", Rowv = F)

setwd("D:/UNI/4TH YEAR/RESEARCH/Codes/Zygotic_Embryogenesis/Plots/figures")
...
q3 = ggplot(data = mod,aes(x = module, y = Day14, fill = module))+
  ...
  ylab("Module-trait relationship") +
  xlab("Modules")
q4 = ggplot(data = mod,aes(x = module, y = Zygotic, fill = module))+
  geom_col()+
  scale_fill_manual(values = clr, guide = F) +
  scale_x_discrete(labels = NULL) +
...
#TOM = TOMsimilarityFromExpr(data, power = 18)
